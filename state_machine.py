...
def show_current_state(round: int, state: CaesarState, show_state: bool):
    if show_state:
        print(f"[STATEMACHINE] Round {round}, entering state: {state}")
...
class CaesarStateMachine:
    def __init__(
        ...
        # check if previous run exists
            ...
            print(f"[RECOVER] Run was not finished, loading existing partial results from {self.logger.log_file}")
    ...
    def load_from_previous_run(self):
        ...
        if self.config.verbose:
            print("[RECOVER] Recoreved log data from previous run: ", saved_log.keys())
    ...
    def compile_logic(self):
        ...
        if self.config.verbose:
            print(f"[COMPILE] Return code: {returncode}")
            print(f"[COMPILE] Compile stdout: {stdout}")
            print(f"[COMPILE] Compile stderr: {err}")
    ...
    def correctness_check_logic(self):
        ...
        """
        if self.config.verbose:
            print(f"[CORRECTNESS] Process {self.process_id} requesting GPU...")

        with self.orchestrator.reserve_gpu() as gpu_id:
            try:
                if self.config.verbose:
                    print(f"[CORRECTNESS] Process {self.process_id} acquired GPU {gpu_id}")
                ...
                if self.config.verbose:
                    print(
                        f"[CORRECTNESS] Worker ({self.process_id}) working on problem {self.work.problem_id}, sample {self.work.sample_id}, result: ",
                        result,
                    )
                ...
                if result is not None:
                    if "cuda_error" in result.metadata: # TODO only cuda error?
                        print(f"[CORRECTNESS] CUDA Error detected, killing process {self.process_id} working on problem {self.work.problem_id}, sample {self.work.sample_id}")
                ...
                if self.config.verbose:
                    print(
                        f"[CORRECTNESS] Process {self.process_id} working on GPU {gpu_id} ({device}) for {work_time:.2f} seconds"
                    )

            except TimeoutError: # TODO this never reaches, does it?
                print(f"[CORRECTNESS] Process {self.process_id} working on GPU {gpu_id} operation timed out")
                self.outcome = StateOutcome.GPUCorrectnessFail
                self.llm_info.eval_result[self.current_k] = kernel_eval.KernelExecResult(
                    compiled=False,
                    correctness=False,
                ...
                print(f"[CORRECTNESS] Process {self.process_id} working on GPU {gpu_id} encountered error: {e}")
                ...
                if self.config.verbose:
                    print(f"[CORRECTNESS] Process {self.process_id} released GPU {gpu_id}")
    ...
    def performance_logic(self):
        ...
        if self.config.verbose:
            print(f"[PERF] Process {self.process_id} requesting GPU...")

        with self.orchestrator.reserve_gpu() as gpu_id:
            try:
                print(f"[PERF] Process {self.process_id} acquired GPU {gpu_id}")
                ...
                if self.config.verbose:
                    print(
                        f"[PERF] Process {self.process_id} working on GPU {gpu_id} ({device}) for {work_time:.2f} seconds"
                    )

            except Exception as e:
                print(f"[PERF] Process {self.process_id} encountered error: {e}")
                ...
                if self.config.verbose:
                    print(f"[PERF] Process {self.process_id} released GPU {gpu_id}")
    ...
    def finish_turn_logic(self):
        ...
        if self.current_k > self.max_k:
            if self.config.verbose:
                print("[FINISH] Finished run, writing DONE file")
