...
def get_eval_results_at_k(
    run_path: str, problem_id: int, sample_id: int, k: int
) -> float:
    log_path = os.path.join(
        run_path, f"problem_{problem_id}", f"sample_{sample_id}", "log.json"
    )
    ...
    if str(turn_num) not in log_json:
        print(
            f"WARNING: turn {turn_num} not found in log for problem {problem_id} sample {sample_id}"
        )
        return -1
        ...
        if "eval_result" not in log_json[str(turn_num)]:
            print(
                f"WARNING: eval_result not found in log for turn {turn_num} for problem {problem_id} sample {sample_id}"
            )
            return -1
        else:
...
def get_overall_best_runtime_for_problem(
    run_path: str, max_k: int, num_samples: int, problem_ids: list[int]
) -> list[float | None]:
    overall_runtimes = []
...
def get_overall_mean_runtime_for_problem(
    run_path: str, max_k: int, num_samples: int, problem_ids: list[int]
) -> list[float | None]:
    """
...
def compute_fast_p_score(
    overall_runtime: list[float | None],
    ...
    baseline_time_array = []
    for problem_id in problem_ids:
        curr_problem_baseline_time = fetch_baseline_time_by_problem_id(
            level=level,
            problem_id=problem_id,
            baseline_time_filepath=baseline_torch_time_filepath,
        ).get("mean", None)
        baseline_time_array.append(curr_problem_baseline_time)

    return fastp(
        is_correct=np.array([x is not None for x in overall_runtime]),
...
def compute_best_fast_p_for_run(
    run_path: str,
    max_k: int,
    num_samples: int,
    level: int,
    ...
    overall_runtime = get_overall_best_runtime_for_problem(
        run_path=run_path, max_k=max_k, num_samples=num_samples, problem_ids=problem_ids
    )
...
def compute_mean_fast_p_for_run(
    run_path: str,
    max_k: int,
    num_samples: int,
    level: int,
    ...
    # get the overall runtime
    overall_runtime = get_overall_mean_runtime_for_problem(
        run_path=run_path, max_k=max_k, num_samples=num_samples, problem_ids=problem_ids
    )
    return compute_fast_p_score(
        overall_runtime=overall_runtime,
...
def main():
    ...
    dataset = KernelBenchDataset(
        dataset=dataset_name_to_dataset["KernelBench/level1-subset"]
    )
    ...
    # 4. used tokens
    input_tok = compute_input_tokens(
        run_path=run_path,
        max_k=max_k,
        num_samples=samples,
        problem_ids=dataset.problem_ids,
    )
    output_tok = compute_output_tokens(
        run_path=run_path,
        max_k=max_k,
        num_samples=samples,
        problem_ids=dataset.problem_ids,
